...
> если вдруг программа `tmux` не установлена в системе ее можно установить с помощью `apt-get`: `sudo apt-get install tmux`

7. __Автостарт__ чтобы программа автоматически запускалась после перезапуска сервера, можно добавить ее вызов в планировщик задач `cron`. Для этого нужно открыть его конфигурационный файл командой `crontab -e`. Добавить туда строчку `@reboot tmux new -d 'node /path/to/your/node/script.js'` и сохранить файл.
...
> После подключению к Raspberry лучше поменять дефолтный пароль `raspberry` на ваш командой `passwd`.

Проверить версию (или вообще присутствие) `node` в системе можно командой `node -v`. В любом случае, его можно обновить, выполнив скрипт установки нода, найденный в интернете:
```
wget -O - https://raw.githubusercontent.com/sdesalas/node-pi-zero/master/install-node-v.last.sh | bash
```

Все остальные команды в системе будут идентичны командам в OS X или тем, что мы выполняли при настройке VPS-сервера, так что можно обратиться к пункту 3.4 текущей инструкции: отправить код программы на Raspberry (`rsync`) и запустить ее в `tmux`.
