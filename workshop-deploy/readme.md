...
7. __Автостарт__ чтобы программа автоматически запускалась после перезапуска сервера, можно добавить ее вызов в планировщик задач `cron`. Для этого нужно открыть его конфигурационный файл командой `crontab -e`. Добавить туда строчку `@reboot tmux new 'node /path/to/your/node/script.js'` и сохранить файл.
